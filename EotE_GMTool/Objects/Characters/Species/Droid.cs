...
namespace EotE_GMTool.Objects.Characters.Species {
    public class Droid : Species {
        public Droid()
        {
            ...
            SpecialAbilities = new List<string>() { "Do not need to eat, sleep, or breathe", "Unaffected by toxins or poisons", "Cybernetic implant cap of 6 instead of their Brawn Rating", "Inorganic: Since droids are inorganic, they do not gain the benefits of recovering with a bacta tank, stimpack, or medicine skill checks.  Droids do recover naturally by resting, as their systems attempt self-repairs.  Otherwise, droids need to be tended to with a Mechanics check, using the same difficulties and results of medicine checks for organic beings. Repair kits can be used to repair damage just like stimpacks are used on organic beings.", "Mechanical being: Droids cannot become Force-Sensitive, nor acquire a force rating by any means.  Droids cannot use force powers, and also cannot be affected by mind-altering force powers."};
            StartingBrawn = 1;
            StartingPresence = 1;
            StartingIntellect = 1;
            StartingCunning = 1;
