...
namespace EotE_GMTool.Objects.Characters.Species {
    public class Human : Species {
        public Human()
        {
            ...
            StartingAgility = 2;
            StartingWillpower = 2;
            SpecialAbilities = new List<string>() { "Humans and near-humans start the game with access to one additional specialization in their chosen career.  This still counts against the three specialization limit.  It does not give the character additional beginning training in additional skills (the player must choose one of the two starting specs to select ranks in starting skills from)"};
            Description =
